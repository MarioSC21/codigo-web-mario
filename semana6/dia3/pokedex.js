 ...
 */
 ...
 const lista = document.querySelector("#uno");

 console.log(lista);
 
async function obetnerLista(){
    pokemonContainer.innerHTML = "";
    const uno= await obtenerPokemones(40).data;
    console.log(uno);
 }

 // vamos a crear una funcion para obtener los datos
 const obtenerPokemones = async (index) => {
   const response = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=${index}`);
 ...
 const color = {
   ...
   blue: "rgba(58, 145, 182, 0.62)",
   brown: "rgba(182, 106, 58, 0.8)",
   purple: "rgba(113, 104, 226, 0.7)",
   pink: "rgba(255, 48, 50, 0.7)",
   yellow: "rgba(229, 216, 0, 0.6)",
   white: "rgba(146, 158, 163, 0.49)",
   gray: "rgba(164, 182, 189, 0.57)"
 };
